...
Where useful, provide: the technical reasoning, what to ask the technician, the next diagnostic test, whether the report should be amended, and a concise customer-ready wording.
...
page = st.sidebar.radio("Navigation", ["Dashboard", "Review report", "Ask Technical Leading Hand", "History", "Company standards", "Plans & setup"])
...
if page == "Dashboard":
...
elif page == "Ask Technical Leading Hand":
    st.title("Ask the Technical Leading Hand")
    st.caption("Ask follow-up questions about the most recent review, paste another report, or use it as a senior HVAC&R sounding board.")

    live_key = st.secrets.get("OPENAI_API_KEY", os.getenv("OPENAI_API_KEY", ""))
    model = st.text_input("AI model", value=st.secrets.get("OPENAI_MODEL", os.getenv("OPENAI_MODEL", "gpt-4.1-mini")), key="chat_model")
    if not live_key:
        st.error("Live chat is not configured. Add OPENAI_API_KEY in Streamlit App settings → Secrets.")

    source = st.radio("Chat context", ["Most recent review", "Paste a different report", "General HVAC&R question"], horizontal=True)
    report_context = ""
    review_context = None

    if source == "Most recent review":
        review_context = st.session_state.get("review")
        if review_context:
            st.success(f"Using saved review #{st.session_state.get('review_id', '—')} as context.")
            with st.expander("View review context"):
                st.json(review_context)
            try:
                if st.session_state.get("review_id"):
                    report_context, _ = get_review(int(st.session_state["review_id"]))
            except Exception:
                report_context = ""
        else:
            st.warning("No review is loaded in this session. Run a review first or choose ‘Paste a different report’.")
    elif source == "Paste a different report":
        chat_upload = st.file_uploader("Upload report for chat", type=["pdf", "docx", "txt", "csv", "md"], key="chat_upload")
        report_context = st.text_area("Technician report", height=260, key="chat_report")
        if chat_upload:
            try:
                report_context = extract_text(chat_upload)
                st.success(f"Extracted {len(report_context):,} characters from {chat_upload.name}.")
            except Exception as exc:
                st.error(str(exc))
    ...
    question = st.chat_input("Ask about the diagnosis, labour, parts, missing tests, report wording or next steps...")
    if question:
        ...
        with st.chat_message("assistant"):
            if not live_key:
                answer = "Live AI is not configured. Add your API key in Streamlit Secrets, then reboot the app."
                st.error(answer)
            else:
                with st.spinner("Reviewing the question..."):
                    try:
                        answer = technical_chat(question, report_context, review_context, model)
                        st.markdown(answer)
                    except Exception as exc:
    ...
    with c2:
        if st.session_state["chat_messages"]:
            ...
            st.download_button("Download chat transcript", transcript, file_name="technical_leading_hand_chat.txt", mime="text/plain")
